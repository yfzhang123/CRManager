...
//        }
    ...
    public Map<String, List<DictionaryValue>> findCacheData() {
        ...
        List<DictionaryType> dictionaryTypes=dictionaryTypeDao.findDictionaryTypeList();
        //根据每个字典类型编码，查询对应字典值列表
        for (DictionaryType dictionaryType : dictionaryTypes) {
            String code = dictionaryType.getCode();
            List<DictionaryValue> dictionaryValues=dictionaryValueDao.FindListByTypeCode(code);
            if(!ObjectUtils.isEmpty(dictionaryValues)){
                resultMap.put(code+"List",dictionaryValues);
            }
        }
        return resultMap;
    }
